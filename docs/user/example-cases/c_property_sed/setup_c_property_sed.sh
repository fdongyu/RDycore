...
if [ "$mach" == "pm-cpu" ]; then
  ...
  batch_filename=${problem_name}.${mach}.c_property_sed.N_${N}.batch
  ...
  then
    # Determine the name of the batch file
    batch_filename=${problem_name}.${mach}-cpu.c_property_sed.N_${N}.batch
  ...
  then
    # Determine the name of the batch file
    batch_filename=${problem_name}.${mach}-gpu.c_property_sed.N_${N}.batch
...
if [ "$problem_name"  == "c_property_sed" ]; then
  yaml_file=c_property_sed.yaml
  exo_file=c_property_sed_dx0.1.exo
  ic_file=c_property_sed_dx0.1_IC.bin
  ic_file_sed=c_property_sed_dx0.1_IC_sed.bin
else
...
then
  ...
  if [ "$problem_name" == "c_property_sed" ]; then
    data_dir=/global/cfs/projectdirs/m4267/dongyu/DATA/c_property_sed/mesh
  else
